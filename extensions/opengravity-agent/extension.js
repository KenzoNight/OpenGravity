...
class AgentViewProvider {
	...
	async handlePrompt(rawText) {
		...
		if (!account) {
			...
			response = 'Ask mode is active. OpenGravity will answer without editing files, running commands, or calling tools once the provider runtime is connected.';
		} else if (mode === 'planning') {
			response = 'Planning mode is active. OpenGravity can prepare a plan, but execution remains blocked until you switch to Agent mode and approve actions.';
		} else {
			const decision = evaluateAction(mode, 'run_task');
			response = `${decision.reason} The orchestration worker will be ported next, so no action was executed.`;
		}
