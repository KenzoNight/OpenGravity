...
function installHeaders() {
	...
	if (local !== undefined) {
		...
		child_process.execFileSync(process.execPath, [node_gyp, 'install', '--dist-url', local.disturl, local.target], { stdio: 'inherit' });
	}

	if (remote !== undefined) {
		// Both disturl and target come from a file checked into our repository
		child_process.execFileSync(process.execPath, [node_gyp, 'install', '--dist-url', remote.disturl, remote.target], { stdio: 'inherit' });
	}
